...
trait ADCModule extends HasDspChainADCIO {
  ...
  val ospReg = RegInit(0.U(64.W))
  val osmReg = RegInit(0.U(64.W))
  val asclkdReg = RegInit(0.U(64.W))
  val extselClkReg = RegInit(false.B)
  val vref0 = RegInit(0.U(64.W))
  val vref1 = RegInit(0.U(64.W))
  val vref2 = RegInit(0.U(64.W))
  val clkgcalReg = RegInit(0.U(64.W))
  val clkgbias = RegInit(0.U(64.W))

  val adcValid = RegInit(false.B)
  val adcSync = RegInit(false.B)

  val osp = wordToByteVec(ospReg)
  val osm = wordToByteVec(osmReg)
  val asclkd = wordToNibbleVec(asclkdReg)
  val extselClk = wordToBoolVec(extselClkReg)
  val clkgcal = wordToByteVec(clkgcalReg)
  ...
  adc.io.extsel_clk0 := extselClk(0)
  adc.io.extsel_clk1 := extselClk(1)
  adc.io.extsel_clk2 := extselClk(2)
  adc.io.extsel_clk3 := extselClk(3)
  adc.io.extsel_clk4 := extselClk(4)
  adc.io.extsel_clk5 := extselClk(5)
  adc.io.extsel_clk6 := extselClk(6)
  adc.io.extsel_clk7 := extselClk(7)
...
class DspChainWithADC(blockConstructors: Seq[Parameters => TLDspBlock])(implicit p: Parameters)
  ...
  val masterStreamNode = AXI4StreamMasterNode(AXI4StreamMasterParameters(
    name = "ADC Master",
    n = 8,
  ))
  // override val streamNode = NodeHandle(masterStreamNode, super.streamNode)
  streamNode := masterStreamNode
  override lazy val module = new LazyModuleImp(this) with ADCModule with RealAnalogAnnotator {
    val (masterOut, _) = masterStreamNode.out.head
    masterOut.valid := adcValid
    masterOut.bits.last := adcSync
    val maxVal = math.pow(2, numInBits - 1).toInt
    masterOut.bits.data := Cat(des_sync.map( d =>
